...
int main(int argc, char* argv[])
{
   ...
   1:	89 e5                	mov    %esp,%ebp
   3:	53                   	push   %ebx
   4:	bb 05 00 00 00       	mov    $0x5,%ebx
   9:	83 e4 f0             	and    $0xfffffff0,%esp
    int pid;
    int numberOfChilds = 5;
	for(int i = 0; i < numberOfChilds; i++) {
        pid = fork();
   c:	e8 aa 02 00 00       	call   2bb <fork>
        if (pid == 0) {
  11:	85 c0                	test   %eax,%eax
  13:	74 1e                	je     33 <main+0x33>
	for(int i = 0; i < numberOfChilds; i++) {
  15:	83 eb 01             	sub    $0x1,%ebx
  18:	75 f2                	jne    c <main+0xc>
            exit();
        }
    }
    ...
    for(int i = 0; i < numberOfChilds; i++) {
        wait();
  1a:	e8 ac 02 00 00       	call   2cb <wait>
  1f:	e8 a7 02 00 00       	call   2cb <wait>
  24:	e8 a2 02 00 00       	call   2cb <wait>
  29:	e8 9d 02 00 00       	call   2cb <wait>
  2e:	e8 98 02 00 00       	call   2cb <wait>
    }
	exit();
  33:	e8 8b 02 00 00       	call   2c3 <exit>
  38:	66 90                	xchg   %ax,%ax
  3a:	66 90                	xchg   %ax,%ax
  3c:	66 90                	xchg   %ax,%ax
  3e:	66 90                	xchg   %ax,%ax
 ...
 611:	a1 6c 0a 00 00       	mov    0xa6c,%eax
{
 ...
 651:	5b                   	pop    %ebx
  freep = p;
 652:	89 15 6c 0a 00 00    	mov    %edx,0xa6c
}
 658:	5e                   	pop    %esi
 659:	5f                   	pop    %edi
 65a:	5d                   	pop    %ebp
 ...
 689:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 68c:	89 15 6c 0a 00 00    	mov    %edx,0xa6c
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 6a9:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 6ac:	8b 3d 6c 0a 00 00    	mov    0xa6c,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 6f7:	73 4f                	jae    748 <malloc+0xa8>
    ...
    if(p == freep)
 6f9:	8b 3d 6c 0a 00 00    	mov    0xa6c,%edi
 ...
 754:	89 72 04             	mov    %esi,0x4(%edx)
      freep = prevp;
 757:	a3 6c 0a 00 00       	mov    %eax,0xa6c
}
 ...
 76e:	66 90                	xchg   %ax,%ax
    base.s.ptr = freep = prevp = &base;
 770:	c7 05 6c 0a 00 00 70 	movl   $0xa70,0xa6c
 777:	0a 00 00 
    base.s.size = 0;
 77a:	bf 70 0a 00 00       	mov    $0xa70,%edi
    base.s.ptr = freep = prevp = &base;
 77f:	c7 05 70 0a 00 00 70 	movl   $0xa70,0xa70
 786:	0a 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 789:	89 f8                	mov    %edi,%eax
    base.s.size = 0;
 78b:	c7 05 74 0a 00 00 00 	movl   $0x0,0xa74
 792:	00 00 00 
    if(p->s.size >= nunits){
